...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_pricing'])) {
    ...
    if (!admin_csrf_verify()) {
        ...
        foreach ($_POST['products'] ?? [] as $id => $fields) {
            ...
            $c1 = (float) str_replace(',', '.', (string) ($fields['carpan1'] ?? 3.6));
            $sabit = (float) str_replace(',', '.', (string) ($fields['sabit'] ?? 0));
            $c2 = (float) str_replace(',', '.', (string) ($fields['carpan2'] ?? 1.45));
            if ($c1 <= 0 || $c2 <= 0 || $sabit < 0) {
                continue;
            }
            $updates[$id] = [
                'carpan1' => $c1,
        ...
        $success = 'Fiyat formülü parametreleri kaydedildi.';
    }
}
...
<head>
  ...
  <style>
    ...
    .row-pending { background: #fff8e7; }
    .status-ok { color: #1f6b42; font-weight: 600; font-size: 0.82rem; }
    .status-warn { color: #9a6700; font-weight: 600; font-size: 0.82rem; }
    .formula-box { background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:12px 16px;margin-bottom:16px;font-size:0.9rem; }
...
<body>
  <div class="box">
    ...
    <?php else: ?>
      ...
      <p>Ölçüye özel sineklik fiyat formülü parametrelerini ve tamir bandı paket ürünlerini buradan yönetin. Değişiklikler sitede birkaç dakika içinde yansır.</p>

      <form method="post">
        <input type="hidden" name="update_pricing" value="1">
        <?= admin_csrf_field() ?>
        <h2>Fiyat Formülü: ((En+Boy) × Çevre Katsayısı + Sabit) × Genel Katsayı</h2>
        <p style="margin-top:0;font-size:0.9rem;">Fiyatlar KDV ve kargo hariçtir. Örnek fiyat 100×150 cm ölçü üzerinden hesaplanmıştır. Kaydettiğinizde sitede birkaç dakikada yansır.</p>
        <div class="formula-box">
          <strong>Formül:</strong>
          ((En + Boy) × <em>Çevre Katsayısı</em> + <em>Sabit</em>) × <em>Genel Katsayı</em>
          <br>
          <span style="color:#666;font-size:0.82rem;">
            Örnek — Dikey Plise, 100×150 cm:
            ((100+150) × 3,6 + 215) × 1,45 = <strong>₺1.617</strong>
          </span>
        </div>
        <table>
          <thead>
          ...
          <tbody>
            <?php foreach ($products as $p):
              $sabit = (float) ($p['sabit'] ?? 0);
              $c1 = (float) ($p['carpan1'] ?? 3.6);
              $c2 = (float) ($p['carpan2'] ?? 1.45);
              $ornek = $sabit > 0 ? (int) round(((100 + 150) * $c1 + $sabit) * $c2) : 0;
              $rowClass = $sabit > 0 ? '' : 'row-pending';
            ?>
              <tr class="<?= $rowClass ?>">
                <td data-label="Ürün">
                  <strong><?= htmlspecialchars($p['name']) ?></strong><br>
                  <small style="color:#888"><?= htmlspecialchars($p['id']) ?></small>
                </td>
                <td data-label="Çevre Katsayısı"><input class="num" type="number" step="0.01" min="0.01" name="products[<?= htmlspecialchars($p['id']) ?>][carpan1]" value="<?= htmlspecialchars((string) $c1) ?>"></td>
                <td data-label="Sabit"><input class="num" type="number" step="1" min="0" name="products[<?= htmlspecialchars($p['id']) ?>][sabit]" value="<?= htmlspecialchars((string) $sabit) ?>"></td>
                <td data-label="Genel Katsayı"><input class="num" type="number" step="0.01" min="0.01" name="products[<?= htmlspecialchars($p['id']) ?>][carpan2]" value="<?= htmlspecialchars((string) $c2) ?>"></td>
                <td data-label="Örnek Fiyat"><?= $sabit > 0 ? '₺' . number_format($ornek, 0, ',', '.') : '—' ?></td>
                <td data-label="Durum"><?= $sabit > 0 ? '<span class="status-ok">✓ Aktif</span>' : '<span class="status-warn">⚠ Girilmedi</span>' ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
        <div class="row-actions">
          <button class="btn" type="submit">Formül Parametrelerini Kaydet</button>
        </div>
      </form>
